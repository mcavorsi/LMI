...
%is quadratically stable under parametric norm-bounded uncertainty if the following LMI in
%A,B,M,B2,N,D_12,C,D22 is feasible. The H infinity optimal controller u = Kx is formed with K = ZP^-1
...
Mat = [A*P+B*Z+P*A'+Z'*B'+B2*B2'+mu*M*M' (C*P+D22*Z)' P*N'+Z'*D12'; C*P+D22*Z -gamma^2*eye(size(C*P)) zeros(size(P*N')); N*P+D12*Z zeros(size(N*P)) -mu*eye(size(P*N'))];
